...
class Item_Trans extends MY_Controller
{
    ...
    public function receive()
    {
        ...
        $item_trans->bin_qty = $this->number_unformat($item_trans->bin_qty);
        $item_trans->qty = $this->number_unformat($item_trans->qty);
        ...
        if ($item_detail = $this->item_detail->get($where)) {
            $item_trans->old_qty = $item_detail->qty;
            $item_trans->new_qty = $item_detail->qty + $item_trans->qty;

            $item_detail->qty += $item_trans->qty;
            //Update ke Item Detail Table
            $id = $item_detail->id;
            $this->item_detail->update($id, $item_detail);
        } else {
            /* Edit - Bugs - 27/05/2016 - BEGIN
            ...
            $item_trans->new_qty = $item_master->qty_eStockCard + $item_trans->qty;
               Edit - Bugs - 27/05/2016 - END */


            // Edit - Bugs - 27/05/2016 - BEGIN
            //Get the item_master information for the associated item code
            $item_master = $this->item_master->get('item_code', $item_trans->item_code);
            $item_trans->old_qty = 0;
            $item_trans->new_qty = $item_trans->old_qty + $item_trans->qty;
            // Edit - Bugs - 27/05/2016 - END
            ...
            $this->item_detail->tambah($item_detail);
        }

        $this->item_trans->tambah($item_trans);
        ...
        $this->item_master->update($item_master->id, $item_master);
    ...
    public function issue()
    {
        ...
        // Cek apakah sudah pernah ada transaksi untuk Item Code, Bin Code yang di-issue
        $where = array(
            'item_code' => $item_trans->item_code,
            'bin_code' => $item_trans->bin_code
        );
    ...
    public function transfer()
    {
        ...
        if (!is_null($item_detail_from_bin)) {
            ...
            $this->item_trans->tambah($item_trans_from_bin);
        ...
        if (!is_null($item_detail_to_bin)) {
            ...
            $this->item_trans->tambah($item_trans_to_bin);
    ...
    {
        $this->data['halaman'] = 'item_adjustment';
        $this->data['main_view'] = 'item_adjustment';
        $this->data['form_action'] = site_url('item_trans/adjustment');


        // Data untuk form.
        if (! $_POST) {
            $item_trans = (object) $this->item_trans->default_value_adjustment;
        } else {
            $item_trans = $this->input->post(null, true);
        }

        // Validasi.
        if (! $this->item_trans->validate('form_rules_adjustment')) {
            $this->data['values'] = (object) $item_trans;
            $this->load->view($this->layout, $this->data);
            return;
        }
        ...
        //$trans_qty = $item_trans->bin_qty - $item_trans->qty;
        if ($item_trans->qty < $item_trans->bin_qty) {
            $trans_qty = $item_trans->qty - $item_trans->bin_qty;          // As if we are issuing item
        }

        if ($item_trans->qty > $item_trans->bin_qty) {
            $trans_qty = $item_trans->qty - $item_trans->bin_qty;          // As if we are receiving item
        }
        ...
        if ($item_detail = $this->item_detail->get($where)) {
            ...
            $item_trans->old_qty = $item_detail->qty;
            $item_trans->new_qty = $item_detail->qty + $trans_qty;

            $item_trans->qty = $trans_qty;

            //Kalau Qty Transaksi = Qty Bin maka data Item Detail di Bin tsb di hapus
            if ($trans_qty < 0) {
                $qty_to_adjust = $trans_qty * -1;

            } else {
                $qty_to_adjust = $trans_qty;
            }

            if ($item_detail->qty == $qty_to_adjust) {
                $this->item_detail->delete($id);
                $this->item_trans->tambah($item_trans);
            } else {
                //Update ke Item Detail Table
            ...
            // Belum pernah ada transaksi di Bin ini
            $this->session->set_flashdata('pesan_error', 'Tidak ada barang ' . $item_trans->item_code . ' di Bin ini. Kembali ke halaman ' . anchor('item_trans/adjustment', 'item adjustment.', 'class="alert-link"'));
            redirect('item_trans/error');
        }
        ...
        if ($this->db->trans_status() == FALSE) {
            $this->session->set_flashdata('pesan_error', 'Transaksi gagal disimpan. Kembali ke halaman ' . anchor('item_trans/adjustment', 'item adjustment.', 'class="alert-link"'));
            redirect('item_trans/error');
        } else {
            $this->session->set_flashdata('pesan', 'Transaksi berhasil disimpan. Kembali ke halaman ' . anchor('item_trans/adjustment', 'item adjustment.', 'class="alert-link"'));
            redirect('item_trans/sukses');
        }
    ...
    */
    ...
    public function sukses()
    {
        ...
        /*
        if ($this->source == 'receive') {
            $this->output->set_header('refresh:1; url='.base_url().'index.php/item_trans/receive');
        } elseif ($this->source == 'issue') {
            $this->output->set_header('refresh:1; url='.base_url().'index.php/item_trans/issue');
        }
        */
    ...
    /*
    public function number_unformat($string)
    {
        $fmt = new NumberFormatter('id_ID', NumberFormatter::CURRENCY);
        return $fmt->parseCurrency($string, $curr);
    }
    */
    ...
    public function _cannot_greater_than_bin_qty()
    {
        ...
        $qty = $this->number_unformat($this->input->post('qty'));
        $bin_qty = $this->number_unformat($this->input->post('bin_qty'));


        //$qty = $this->number_unformat($qty);
        //$bin_qty = $this->number_unformat($bin_qty);
        //echo $qty;
        //echo $bin_qty;
        //$this->form_validation->set_message('_cannot_greater_than_bin_qty', $qty.'+'.$bin_qty);
        ...
        public function _qty_adjustment_valid()
    {
        ...
        if ($qty == $bin_qty) {
            $this->form_validation->set_message('_qty_adjustment_valid', 'Final Qty harus berbeda dari Bin Qty.');
            return false;
        }
